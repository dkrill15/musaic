...
TILE_SIZE      = 60		# height/width of mosaic tiles in pixels
...
class TileProcessor:
	...
	def __process_tile(self, tile_path):
		try:
			img = Image.open(requests.get(tile_path.rstrip(), stream=True).raw)
	...
	def get_tiles(self):
		...
		with open(self.tiles_directory, 'r') as f:
			for line in f:
				large_tile, small_tile = self.__process_tile(line)
				if large_tile:
